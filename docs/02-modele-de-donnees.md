...
possède une ligne active dans `merchant_staff`. C'est la seule lecture compatible avec le design
(C4). Si Q2 tranche pour l'exclusivité, on ajoutera `primary_role` — la migration reste triviale.
...
Index : `unique(join_code)`, `(status)`, `(city, category)` pour la découverte future.

---

## 4. `merchant_staff` — l'équipe

Table pivot qui porte **à la fois** le droit d'accès et l'attribution des actions (règle 5).
...
| `display_name` | `text` not null | « Karim » — ce qui s'affiche dans l'historique |
| `revoked_at` | `timestamptz` | révocation sans perte d'historique |

Contraintes : `unique(merchant_id, profile_id)`, plus un index partiel unique garantissant
**au moins un `owner` actif** par commerce.
Index : `(profile_id) where revoked_at is null` — c'est la requête faite à chaque appel de RLS.
...
| `staff_id` | `uuid` → `merchant_staff` | qui a agi (règle 5) |
| `staff_display_name` | `text` not null | **instantané** — reste juste même si le membre part |
...
Les instantanés (`staff_display_name`, `threshold_at_time`, `reward_label`) sont volontairement
...
### Assistants de RLS
```
auth_is_staff_of(p_merchant uuid) → boolean      -- STABLE, coupe la récursion RLS
...
2. vérifie que l'appelant est membre actif du personnel du commerce du programme — **sinon `FORBIDDEN`**
...
| Table | Client | Personnel du commerce | Écriture directe |
|---|---|---|---|
| `profiles` | la sienne | vue restreinte via fonction | soi-même |
| `merchants` | lecture publique des `active`, colonnes limitées | son commerce | personnel `owner`/`manager` |
| `merchant_staff` | ✗ | son commerce | `owner` |
| `programs` | les `active` des commerces où il est membre | les siens | personnel |
...
2. Les politiques passent toutes par `auth_is_staff_of()`, déclarée `STABLE SECURITY DEFINER`, pour
   éviter la récursion RLS (une politique sur `merchant_staff` qui lirait `merchant_staff`).
...
- `programs.status = 'archived'` et `merchant_staff.revoked_at` : désactivation sans suppression,
  donc sans trou dans l'historique.
